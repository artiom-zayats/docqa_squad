...
class ParagraphAndQuestionSpec(object):
    ...
    def __init__(self, batch_size, max_question_words=None,
                 ...
                 max_num_of_choices=None, max_num_choices_words=None):
        #pdb.set_trace()
        ...
        self.max_num_choices_words = max_num_choices_words
        self.max_word_size = max_word_size
        self.max_batch_size = max_batch_size
        self.max_num_of_choices = max_num_of_choices
        #pdb.set_trace()
    ...
    def __add__(self, o):
        return ParagraphAndQuestionSpec(
            ...
            max_or_none(self.max_num_of_choices, o.max_num_of_choices),
            max_or_none(self.max_num_choices_words, o.max_num_choices_words)
        )
...
def build_spec(batch_size: int,
               max_batch_size: int,
               data: List[ContextAndQuestion], num_choices: int) -> ParagraphAndQuestionSpec:
    max_ques_size = 0
    max_word_size = 0
    max_para_size = 0
    max_choice_size = 0
    for data_point in data:
        ...
        if data_point.choices is not None:
            max_choice_size = max(max_choice_size, max([len(choice) for choice in data_point.choices]))
            max_choice_word_size = max([len(word) for choice in data_point.choices for word in choice])
            max_word_size = max(max_word_size,max_choice_word_size)
    return ParagraphAndQuestionSpec(batch_size, max_ques_size, max_para_size,
                                    max_word_size, max_batch_size, num_choices,
                                    max_choice_size)
...
class ParagraphAndQuestionDataset(ListDataset):
    ...
    def __init__(self, data: List, batching: ListBatcher, unfiltered_len: Optional[int]=None, num_choices: int=None):
...
class ParagraphQaTrainingData(TrainingData):
    ...
    def get_train(self) -> Dataset:
        self._load_data()
        return ParagraphAndQuestionDataset(self._train, self.train_batcher, num_choices=self.num_choices)
    ...
    def get_eval(self) -> Dict[str, Dataset]:
        self._load_data()
        eval_sets = dict(train=ParagraphAndQuestionDataset(self._train, self.eval_batcher, self._train_len, num_choices=self.num_choices))
        if self._dev is not None:
            eval_sets["dev"] = ParagraphAndQuestionDataset(self._dev, self.eval_batcher, self._dev_len, num_choices=self.num_choices)
        return eval_sets
