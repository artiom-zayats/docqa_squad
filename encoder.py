...
#ading new Context #vz
#from docqa.race.race_data import DocParagraphAndQuestion
"""
...
class MultiChoiceAnswerEncoder(AnswerEncoder):
    '''Encoder for MulitChoice task with answer 0-n'''
    def __init__(self):
    ...
    def init(self, batch_size, num_of_choices):
        self.correct_answer = tf.placeholder('int32',[batch_size, num_of_choices], name = 'correct_answer' )
        #self.correct_answer = tf.placeholder('int32',[batch_size], name = 'correct_answer' )

    def encode(self,batch_size,num_of_choices,batch) -> Dict:
        correct_answer = np.zeros((batch_size, num_of_choices), dtype=np.int32)
        #correct_answer = np.zeros((batch_size, 1),dtype=np.int32)
       #pdb.set_trace() #need to fix
        for doc_ix, doc in enumerate(batch):
            if doc.answer is None:
                continue
            #pdb.set_trace()
            correct_answer[doc_ix, doc.answer] = 1
            #correct_answer[doc_ix] = doc.answer
        #pdb.set_trace()
        return {self.correct_answer: correct_answer}
...
class DocumentAndQuestionEncoder(Configurable):
    ...
    def __init__(self,
        ...
        self.max_choices_word_dim = None
        self.max_char_dim = None
        self.max_choice_dim = None
        ...
        self.choices_features = None
        self.choices_words = None
        self.choices_chars = None
        self.choices_len = None
        self.choices_word_len = None

    @property
    def version(self):
        return 3
    #az need to add choices
    def init(self, input_spec: ParagraphAndQuestionSpec, len_op: bool,
        ...
        #pdb.set_trace()
        self.max_choices_dim = input_spec.max_num_of_choices
        self.len_opt = len_op
        ...
        if not self.len_opt:
            ...
            self.max_choices_word_dim = input_spec.max_num_choices_words
            
            if self.max_choices_word_dim is None or self.max_choices_dim is None :
                raise ValueError()
        ...
        n_choices_words = self.max_choices_word_dim
        n_choices = self.max_choices_dim
        ...
        self.choices_words = tf.placeholder('int32', [batch_size, n_choices, n_choices_words], name='choices_words')
        self.choices_len = tf.placeholder('int32', [batch_size, n_choices], name='choices_len')
        ...
        if self._char_emb:
            ...
            self.choices_chars = tf.placeholder('int32', [batch_size,n_choices ,n_choices_words, self.max_char_dim], name='choices_chars')
            self.choices_word_len = tf.placeholder('int32', [batch_size, n_choices, n_question_words], name='choices_len')
        ...
        self.answer_encoder.init(batch_size, n_choices)
    ...
    def get_placeholders(self):
        ...
        return [x for x in
                [self.question_len, self.question_words, self.question_chars, self.question_features,
                 ...
                 self.question_word_len, self.context_word_len, self.choices_word_len,

                 self.choices_len, self.choices_words, self.choices_chars, self.choices_features ]
                if x is not None] + self.answer_encoder.get_placeholders()
    #chaning Context to DocPara
    def encode(self, batch: List[ContextAndQuestion], is_train: bool):
        ...
        context_word_dim, ques_word_dim, choices_word_dim , max_char_dim = \
            self.max_context_word_dim, self.max_ques_word_dim,self.max_choices_word_dim, self.max_char_dim
        ...
        #pdb.set_trace()
        choices_num_max = max([len(x.choices) for x in batch])
        choices_len = np.array([[len(x.choices[i]) for i in range(len(x.choices))] for x in batch], dtype='int32')
        #pdb.set_trace()
        if choices_word_dim is not None:
            if choices_len.max() > choices_word_dim:
                raise ValueError("Have a choices of len %d but max choice dim is %d" %
                                 (choices_len.max(), choices_word_dim))
        else:
            choices_word_dim = choices_len.max()

        feed_dict[self.context_len] = context_len
        feed_dict[self.question_len] = question_len
        feed_dict[self.choices_len] = choices_len
        #pdb.set_trace()
        ...
        if self._word_embedder is not None:
            ...
            choices_words = np.zeros([batch_size, choices_num_max, choices_word_dim], dtype='int32')
            feed_dict[self.context_words] = context_words
            feed_dict[self.question_words] = question_words
            feed_dict[self.choices_words] = choices_words
            #pdb.set_trace()
        else:
            question_words, context_words, choises_words = None, None, None
        ...
        if self._char_emb is not None:
            ...
            choices_chars = np.zeros([batch_size, choices_num_max, choices_word_dim, max_char_dim], dtype='int32')
        ...
        for doc_ix, doc in enumerate(batch):
            ...
            for choice_ix, choice in enumerate(doc.choices):
                for word_ix, word in enumerate(choice):
                    if self._word_embedder is not None:
                        if query_once:
                            ix = doc_mapping.get(word)
                            if ix is None:
                            ...
                            ix = self._word_embedder.context_word_to_ix(word, is_train)
                        choices_words[doc_ix, choice_ix, word_ix] = ix
                    if self._char_emb is not None:
                        choices_word_len[doc_ix, choice_ix, word_ix] = min(self.max_char_dim, len(word))
                        for char_ix, char in enumerate(word):
                            if char_ix == self.max_char_dim:
                                break
                            choices_chars[doc_ix, choice_ix, word_ix, char_ix] = self._char_emb.char_to_ix(char)
        #pdb.set_trace()
        # Answer placeholders az
        #feed_dict.update(self.answer_encoder.encode(batch_size, context_len, context_word_dim, batch))
        feed_dict.update(self.answer_encoder.encode(batch_size,self.max_choices_dim, batch))
        #def encode(self,batch_size,num_of_choices,batch) -> Dict:
        #pdb.set_trace()
        ...
        if self.word_featurizer is not None:
            ...
            choices_word_features = np.zeros((batch_size, choices_num_max, choices_word_dim, self.word_featurizer.n_context_features()))
            
            for doc_ix, doc in enumerate(batch):
                q_f, c_f = self.word_featurizer.get_features(doc.question, doc.get_context())
                #az need to change word_featurezer
