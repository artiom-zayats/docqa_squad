...
class SpanEvaluator(Evaluator):
    ...
    def tensors_needed(self, prediction):
        ...
        #return {str(b): prediction.get_best_span(b)[0] for b in self.bound} #az
...
class MultiChoiceEvaluator(Evaluator):
    '''MultiChoice Eval for Race Data'''
    def __init__(self,num_of_choices):
        self.num_of_choices = num_of_choices
    ...
    def evaluate(self, data: List[ContextAndQuestion], true_len, **kwargs):
        ev = Evaluation({})
        #pdb.set_trace()
        scores = race_scores(data, kwargs['pred_answers'].flatten())
        answers_per_sample = race_per_sample(data, kwargs['pred_answers'].flatten())

        scores_out = {
                "race data accuracy": scores[0],
            }

        per_sample_out = {
                "question_id" : answers_per_sample[0],
                "correct answer" : answers_per_sample[1],
                "correct index" : answers_per_sample[2],
                "predictied answer" : answers_per_sample[3],
                "predictied index" : answers_per_sample[4],
                "is correct" : answers_per_sample[5],
            }

        ev.add(Evaluation(scores_out,per_sample_out))
        #pdb.set_trace()
        return ev


def race_scores(data: List[ContextAndQuestion],
                       pred_answers):
    accuracy = 0
    if len(pred_answers) != 0 :
        pred_bool = np.zeros(len(pred_answers))
        for ix in range(len(pred_answers)):
            if (pred_answers[ix] == data[ix].answer):
                pred_bool[ix] = 1
            else :
                pred_bool[ix] = 0
        #pdb.set_trace()        
        accuracy = pred_bool.sum()/len(pred_answers)
    return [accuracy]
        
def race_per_sample(data: List[ContextAndQuestion],
                       pred_answers):
    idx = []
    correct = []
    correct_ix = []
    ...
    is_correct  = []
    for ix in range(len(pred_answers)):
        idx.append(data[ix].question_id)
        correct.append(data[ix].choices[data[ix].answer])
        correct_ix.append(data[ix].answer)
        predicted.append(data[ix].choices[pred_answers[ix]])
        predicted_ix.append(pred_answers[ix])
        is_correct.append(True if pred_answers[ix] == data[ix].answer else False)


    return [idx, correct, correct_ix , predicted , predicted_ix , is_correct]
...
class EvaluatorRunner(object):
    ...
    def set_input(self, prediction: Prediction):
        ...
        pdb.set_trace()
        #tensors_needed = tensors_needed[0]
