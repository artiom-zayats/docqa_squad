...
class MultiChoicePredictor(SequencePredictionLayer):
    def __init__(self,num_of_choices,init: str="glorot_uniform"):
        self.num_of_choices = num_of_choices
        self.init = init
        self.recurrent_layer = CudnnGru(100)

    def apply(self, is_train, context_embed, choices_embed, answer, context_mask=None , choices_mask = None):
        '''
        #pdb.set_trace()
        init_fn = get_keras_initialization(self.init)
        
        with tf.variable_scope("calculate_logits_step1"):
            logits = fully_connected(context_embed, 1,
                                      activation_fn=None, weights_initializer=init_fn)

        with tf.variable_scope("squeeze_logits"):
            logits = tf.squeeze(logits, squeeze_dims=[2])
        '''
        #loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=answer[0]) #check dimm
        #pdb.set_trace()

        
        #loss = tf.identity(tf.reduce_mean(loss), name = "loss")
        
        h = context_embed.shape[-1]
        W = tf.get_variable('W_Bi_Attention', shape=[h,h], dtype=tf.float32,
       regularizer=None, trainable=True)

        M = tf.expand_dims(tf.tensordot(context_embed,W,axes = [[1],[1]]),1)
        logits = tf.reduce_sum(tf.multiply(choices_embed,M) , 2 , name = 'logits')
        ...
        loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=answer[0])
        #pdb.set_trace()
        ...
        tf.add_to_collection(tf.GraphKeys.LOSSES, loss)
        #pdb.set_trace()
        with tf.variable_scope("predict_answer"):
            return MultiChoicePrediction(probs,logits)


class MultiChoicePrediction(Prediction):
    """ Individual logits and probabilities """

    def __init__(self, prob, logits):
        self.prob = prob
        self.logits = logits

    def get_prediction(self):
        #pdb.set_trace()
        top_k = tf.argmax(self.prob, axis = 1,name = 'predict_answers')
        best_score = tf.reduce_max(self.prob, axis=1 , name = 'best_score')
        #pdb.set_trace()
        #return dict(top_k[1] , best_score)
        return dict(pred_answers = top_k, best_scores = best_score)


    def get_span_scores(self):
        return tf.exp(tf.expand_dims(self.logits, 2))
