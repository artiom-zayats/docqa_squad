...
import code
import pdb
from docqa.azayats_scripts import send_email , create_train_dev_plot
...
def _train_async(model: Model,
                    ...
                    for name, data in eval_datasets.items():
                        ...
                        if train_params.best_weights is not None and name == train_params.best_weights[0]:
                            ...
                            if cur_best is None or val > cur_best:
                                epoch_best = epoch
                                send_email('epoch: '+str(epoch_best)+ 'acc: '+ str(val),'New Best')
                                ...
                                cur_best = val
                                if(cur_best > 0.37):
                                    email_text ='Best accuracy for dev data: '+('%.3f'%cur_best)+' <br> On epoch n: '+str(epoch_best)+' out of: '+str(train_params.num_epochs) + ' <br> Folder: ' + str(out.save_dir)
                                    email_title = 'Good News EveryOne!'
                                    send_email(email_text,email_title)

                            print('Current accuracy for dev data: ' + ('%.3f'%val))
                            print('Best accuracy for dev data: ' + ('%.3f'%cur_best)+ 'on epoch n:' + str(epoch_best))
                        else:
                            val_train = evaluation.scalars[ train_params.best_weights[1]]
                            train_acc.append(val_train)
                            print('Current accuracy for train data: ' + ('%.3f'%val_train))
    ...
    email_text = 'Finished '+str(train_params.num_epochs)+' Best accuracy for dev data: '+('%.3f'%cur_best)+' <br> On epoch n: '+str(epoch_best)+' <br> Acc for train data last: '+('%.3f'%val_train)+' <br> Folder: '+str(out.save_dir)
